...
from preprocess_data import CustomDataset, SpeakerDS
...
class HypHawkes(torch.nn.Module):
    def __init__(self, dimensions, bs, attention_type="general", c=1.0):
    ...
    def forward(self, query, context, delta_t, c):
        ...
        li = []
        for i in range(bs):
            tmp = pmath_geo.weighted_midpoint(mix[i], k=c, dim=0)
            li.append(tmp)
    ...
    def forward(self, inputs, timestamps, hidden_states, c, reverse=False):
        ...
        for s in range(seq):
            c_s1 = pmath_geo.expmap0(
                torch.tanh(
                    pmath_geo.logmap0(pmath_geo.mobius_matvec(self.W_d, _c, k=c), k=c)
                ),
                k=c,
            )  # short term mem
            c_s2 = pmath_geo.mobius_pointwise_mul(
                c_s1, timestamps[:, s : s + 1].expand_as(c_s1), k=c
            )  # discounted short term mem
            ...
            f = pmath_geo.logmap0(
                one_rnn_transform(W_f, h, U_f, inputs[:, s], c), k=c
            ).sigmoid()
            i = pmath_geo.logmap0(
                one_rnn_transform(W_i, h, U_i, inputs[:, s], c), k=c
            ).sigmoid()
            o = pmath_geo.logmap0(
                one_rnn_transform(W_o, h, U_o, inputs[:, s], c), k=c
            ).sigmoid()
            c_tmp = pmath_geo.logmap0(
                one_rnn_transform(W_c_tmp, h, U_c_tmp, inputs[:, s], c), k=c
            ).sigmoid()
...
if __name__ == "__main__":
    ...
    dl = next(iter(loader))
    # inp_tensor = torch.cat((dl["tweet_features"], dl["temporal_tweet_features"]), 1).to(
    #     "cuda"
    # )
    inp_tensor = dl["temporal_tweet_features"]
    print(inp_tensor.shape)
    dates = dl["normal_timestamp"]
    dates_inv = dl["inverse_timestamp"]
    bs = inp_tensor.shape[0]
    model = HYPHEN(
        768, bs, bs, attn_type="vanilla", do_hyp_lstm=False, learnable_curvature=False
    ).to("cuda")
    tmp = model(inp_tensor, dates, dates_inv)
    print(tmp.shape)
